...
const App: React.FC = () => {
    ...
    // 页签数据
    const { panesTab, setPanesTab, setBreadCrumbs } = usePanesState();
    // 页签选中key
    const [activeKey, setActiveKey] = useState(panesTab.length > 0 ? panesTab[0].key : '');
    // 监听路由变化
    useEffect(() => {
        // 根据路由获取菜单对象，及面包屑
        ...
        const index = panesTab.findIndex(
        ...
        if (index > -1) {
            ...
            setActiveKey(newPanesTab[index].key)
            return
        }
        // 添加新tab并保存起来
        const newPane = {
    ...
    const onTabClick = (key: string, e: React.MouseEvent | React.KeyboardEvent) => {
        ...
        if (target && target.classList && target.classList.contains('ant-dropdown-menu-title-content')) return;
        setActiveKey(key);
                ...
                items={panesTab.map(pane => ({
                    ...
                    >
                        <span onContextMenu={(e) => preventDefault(e, pane)}>
                             {reloadPath &&
                             pane.path === fullPath && (
                                 <SyncOutlined title="刷新" spin={true} />
                             )}
                            {pane.label}
                        </span>
                    </Dropdown>,
